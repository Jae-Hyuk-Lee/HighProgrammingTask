...
//player struct 안에 저장되어있는 is_Alive bool을 이용해
//플레이어가 살아있는지 체크. 그 후, 살아있는 플레이어만을 이용해
//제비 게임을 실행해야함.
...
void jebi_init() {
    ...
    while (player[player_turn].is_alive == false) {
        player_turn += 1;
...
int get_result() {
    ...
    if (result[current_pos] == 1) {
        ...
        dialog(str123, strlen(str123));

        player[player_turn].is_alive = false;
        n_jebi_alive -= 1;
        return 1;
    }
    else {
